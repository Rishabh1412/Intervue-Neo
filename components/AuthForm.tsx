  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    try{
      if(type === "sign-up"){
        ...
        if(!result?.success){
          toast.error(result?.message || "Could not create a user. Please try again later.");
          return;
        }
        ...
        const idToken = (await userCredentials.user.getIdToken()).toString();
        if(!idToken){
      ...
      toast.error("An error occurred while submitting the form. Error : " + error);
    }
    console.log(values);
  }
    ...
    <div className="card-border lg:min-w-[566px]">
      <div className="flex flex-col gap-6 card py-14 px-10">
        <div className="flex flex-row gap-2 justify-center">
          ...
          <h2 className="text-primary-100">Intervue Neo</h2>
        </div>
